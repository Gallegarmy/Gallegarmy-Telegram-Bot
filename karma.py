    ...
    if context.args:
        ...
        if usuario != str(update.message.from_user.username):
            ...
            if len(usuarios) == 0:
                ...
                await context.bot.send_message(chat_id=update.effective_chat.id, text=f'+1 Karma para {usuario.lower()}', message_thread_id=thread_id)
            else:
...
async def kshow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if len(usuarios) == 0:
            ...
            cursor.execute(SQLCreateuser, (usuario.lower(),))
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f'El Karma de {usuario.lower()} es 0', message_thread_id=thread_id)
        else:
            SQLShowKarma = ("SELECT karma FROM karma WHERE user = ?")
            cursor.execute(SQLShowKarma, (usuario.lower(),))
            karma = cursor.fetchall()
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f'El Karma de {usuario.lower()} es {karma[0][0]}', message_thread_id=thread_id)
        database.commit()
        database.close()
    else:
