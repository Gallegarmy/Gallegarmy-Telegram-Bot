...
async def cerveza(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if datetime.now() > first_friday_of_month(current_year, current_month):
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"Fecha: {first_friday_of_month(next_month_year, next_month).strftime('%d-%m-%Y')}\n\nHora: 19:00\n\nUbicación: Cervecería Yate's (Rúa Juan Díaz Porlier, 6, 15009 A Coruña)")
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"Fecha: {first_friday_of_month(current_year, current_month).strftime('%d-%m-%Y')}\n\nHora: 19:00\n\nUbicación: Cervecería Yate's (Rúa Juan Díaz Porlier, 6, 15009 A Coruña)")