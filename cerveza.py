...
async def cerveza(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if datetime.now() > first_friday_of_month(current_year, current_month):
        ...
        formatted_date= first_friday_of_month(next_month_year, next_month).strftime('%d-%m-%Y')
        ...
        cursor = database.cursor()
        SQLEvents = ("SELECT * FROM events WHERE fecha = ?")
        formatted_date = first_friday_of_month(current_year, current_month).strftime('%d-%m-%Y')
        cursor.execute(SQLEvents, (formatted_date,))
        event = cursor.fetchone()
        if event is None:
            SQLCreateEvent = ("INSERT INTO events (fecha, hora, link, lugar, maps) VALUES (? , '19:00', 'https://www.meetup.com/es-ES/gallegarmy/', 'Fire Capitano (Rúa Federico García, 2, 15009 A Coruña)', 'https://maps.app.goo.gl/ao8dejkwv74QV6eb7')")
...
async def cervezahora(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if str(update.message.from_user.username) in admins:
            ...
            database.commit()
            database.close()
            await context.bot.send_message(chat_id=update.effective_chat.id, text='Se ha modificado la hora del evento', message_thread_id=thread_id)
...
async def cervezalugar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if str(update.message.from_user.username) in admins:
            ...
            database.commit()
            database.close()
            await context.bot.send_message(chat_id=update.effective_chat.id, text='Se ha modificado el lugar del evento', message_thread_id=thread_id)
...
async def cervezamapa(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if str(update.message.from_user.username) in admins:
            ...
            database.commit()
            database.close()
            await context.bot.send_message(chat_id=update.effective_chat.id, text='Se ha modificado el mapa del evento', message_thread_id=thread_id)
...
async def cervezaasistencia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if str(update.message.from_user.username) in admins:
            ...
            database.commit()
            database.close()
            await context.bot.send_message(chat_id=update.effective_chat.id, text='Se ha modificado la asistencia del evento', message_thread_id=thread_id)
