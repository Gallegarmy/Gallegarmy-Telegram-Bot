...
async def add_quote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if quoted_msg is None:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Necesítase unha mensaxe para citar.", message_thread_id=thread_id)
        return    
...
async def random_quote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) == 1:
        ...
        if len(quotes) > 0:
            ...
            for quote in quotes:
                user_quotes+=f"\n{quote[0]}"
            await context.bot.send_message(chat_id=update.effective_chat.id, text=user_quotes, message_thread_id=thread_id)
            database.close()
            return
        else:
            await context.bot.send_message(chat_id=update.effective_chat.id, text="Non se atoparon quotes dese usuario", message_thread_id=thread_id)
            database.close()
            return
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=random_quote, message_thread_id=thread_id)
        database.close()
        return
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Pasáronse máis argumentos dos permitidos", message_thread_id=thread_id)
        database.close()
        return